...
CA_FILE = "AmazonRootCA1.pem"   # use your actual file name
...
def on_connect(client, userdata, flags, rc, properties=None):
    ...
    state = {"gateway_id":"edgeblr01","status":"online","ts":datetime.now(timezone.utc).isoformat()}
...
mqttc.tls_set(ca_certs=CA_FILE, certfile=CERT_FILE, keyfile=KEY_FILE, tls_version=ssl.PROTOCOL_TLSv1_2)
...
while True:
    ...
    if epoch % 30 == 0:
        state = {"gateway_id":"edgeblr01","status":"online","ts":datetime.now(timezone.utc).isoformat()}
        mqttc.publish(TOPIC_STATE, json.dumps(state), qos=1, retain=True)
